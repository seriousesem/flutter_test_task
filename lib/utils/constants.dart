
abstract class AppKeys{
  static const String userId = 'user_id';
  static const String usersJson = 'users_json';
}

abstract class ApiEndPoints{
  static const String apiBaseUrl = 'https://reqres.in/';
  static const String fetchUsers = 'api/users?page=';
  static const String fetchUserDetails = 'api/users/';
}

abstract class ScreenTitles{
  static const String app = 'Test task';
  static const String homeScreen = 'Home screen';
  static const String userDetails = 'User details';
}

abstract class UserDetailsInscription{
  static const String userId = 'User id:';
  static const String userFirstName = 'User first name:';
  static const String userLastname = 'User last name:';
  static const String userEmail = 'Email:';
}


